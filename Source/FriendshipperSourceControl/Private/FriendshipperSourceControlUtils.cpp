...
#define GIT_USE_CUSTOM_LFS 1
	...
	{
	public:
		FFriendshipperStatusFileMatcher(const FString& InAbsoluteFilename)
	...
	bool RunUpdateStatus(const FString& InRepositoryRoot, const TArray<FString>& InFiles, EForceStatusRefresh FetchRemote, TMap<FString, FFriendshipperSourceControlState>& OutStates)
	{
		...
		const TArray<FString>& RepoFiles = InFiles.FilterByPredicate([InRepositoryRoot](const FString& File)
			{
				return File.StartsWith(InRepositoryRoot);
			});
	...
	*/
	...
	FName OtelTracerNameForThread()
	{
		...
		FString TracerName = FString::Printf(TEXT("FriendshipperSourceControl_%u"), ThreadId);
		return FName(TracerName);
	}
